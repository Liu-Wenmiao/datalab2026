...
int samesign(int x, int y) {
     ...
     *                   （即课堂上讲的的"符号扩展"），得 0 或全 1，异或为 0 即同号 */
    if ((!x) && (!y)) {
        return 1;
    }
    if (!(x && y)) {
 ...
 */
int logtwo(int v) {
    /* 求最高位 1 的下标，本质同课堂上讲的例题"表示 X 最少要多少位"里的二分定位法：
     * 先问"高 16 位还有没有 1"，有就把范围折半到那一半、并记下权重 16；
     * 再问 8、4、2、1，每步把范围折半，五步锁定最高位。
     *
     * 与课堂上讲的写法相比，有两处必要的替换：
     *   1) 课堂上讲的用 !!(x >> 16) 把"非零"压成 1，但 ! 不在本题白名单，改用 > 0；
     *   2) 课堂上讲的用 + 累加权重，但 + 也不在白名单，改用 |。
     *      这一步成立是因为五段权重 16/8/4/2/1 两两不重叠——正是课堂上讲的
     *      "提取和拼装"里说的"各段移到自己的位置再用 | 拼起来（前提是各段不重叠）"。
     * 首段写 r = b << 4 而不是 r | (b << 4)：r 初值为 0，那个 | 是多余的，
     *
     * 关键点：省掉它正好把运算符计数压到上限 25。
     *
     * 题目保证 v > 0；v <= 1（含负数）时五步全落空，自然返回 0。如有额外要求就会超上限。 */
    int r = 0;
    int b;
...
int byteSwap(int x, int n, int m) {
    /* 课堂上讲的"提取和拼装"的套路：
     *   一字节3bit，<<3
     *   提取第 p 字节 = (x >> (p<<3)) & 0xFF       —— 先右移到最低，再 & 掩码清除多余高位
...
unsigned reverse(unsigned v) {
     ...
     * (v & 1) 就是课堂上讲的"取第 n 位"公式 (x >> n) & 1 在 n = 0 时的形式。 */
    unsigned r = 0;
    int i = 32;
    while (i) {
        r = (r << 1) | (v & 1);
...
int logicalShift(int x, int n) {
     ...
     * 加上课堂上讲的技巧表里的"全 1 掩码" ~0 恰好得到全 1；n >= 1 时 1<<(31-n) 是正数，
     * 左移一位恰为 1<<(32-n)。末尾的 + ~0 则是把00……01000……00减去1变成低位一堆1，高位一堆0的掩码。 */
    int mask = ((1 << (32 + ~n)) << 1) + ~0;
    return (x >> n) & mask;
}
...
int leftBitCount(int x) {
    /* 与课堂上讲的 bitCount 的"折半并行求和"同源，只是方向相反：那边是自底向上把
     * 相邻位两两合并，这边是自顶向下分段确认，步长取 16、8、4、2、1、1。
     * 每步考察"紧接着已确认前缀的那 s 位"是否全为 1：右移 (32-c-s) 位把它移到
     * 最低端，用 s 位掩码取出（课堂上讲的"取宽度 w 的字段：(x>>p) & ((1<<w)-1)"），
     * 再与"全 1"比较，成立就把 s 累加进 c。
     *
     * 三个容易踩的点：
     *   1) x 有符号，算术右移会复制符号位，取出该段后必须 & 掩码，
     *      否则 (x >> 16) 对负数得 0xFFFFFFFF 而非 0x0000FFFF，比较恒不成立；
     *   2) 本题没有 ==、-：判等用 !(a ^ b)（相等时异或为 0）；
     *   3) 五步合计只覆盖 31 位，最低位由最后一行按课堂上讲的"五个必背公式"第 1 条
     *      (x >> n) & 1 单独取出。
     *
...
unsigned float_i2f(int x) {
     ...
     * 这里要绕开课堂上讲的的"补码著名的坑"：-INT_MIN == INT_MIN，取负后没有对应的
     * 正数。把幅值交给 unsigned 变量承载，负号1000 0000 0000 0000 0000 0000 0000 0000 得到的位模式是 0x80000000，
    ...
    /* 最高位 1 的下标 e 决定阶码：exp = e + 127（阶码用，移码存） */
    int e = 0;
    unsigned t = mag;
    while (t > 1) {
        t = t >> 1;
    ...
    /* 用掩码0111 1111 1111 1111 1111 1111 1111 1111丢掉隐含的最高位 1，其后 23 位为尾数。
    ...
    /* 课堂上讲的"偶数舍入"（就近舍入、逢半取偶）：先与半个 ulp（round 的最高位）
...
unsigned floatScale2(unsigned uf) {
    /* 按课堂上讲的的"Float 的阶码分区"分四类：
     *   0   ：0 或非规格化数（0.几），乘 2 即整体左移一位（符号位单独保留），
     *         非规格化数可借此自然进位，成规格化数（1.几）
...
int float64_f2i(unsigned uf1, unsigned uf2) {
     ...
     * 先用"提取和拼装"取出隐含的 1 与其后的 31 位尾数，得到一个 32 位窗口 val：
     * 最高位（bit31）就是那个隐含的 1，其后是尾数的高 31 位。
     * 于是整体再右移 (1023 + 52 - 32 -exp) 位，等价于把 53 位有效数字右移 (1023 + 52 -exp) 位，
     * 移出去的部分正是小数部分——"向零取整"天然成立，无需额外判断。
     *
     * 边界：exp < 1023 表示 |v| < 1，返回 0；exp > 1054 表示 |v| >= 2^31，返回0x80000000。
     * 移位之后还必须再判一次 val 是否越界——漏掉这步，负数的 -val 会符号回绕，
     * 例如 -2^31 - 1 会错误地返回一个正数。 */
    int sign = uf2 >> 31;
